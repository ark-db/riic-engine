...
>
	...
	<div class="buttons">
		{#if level < maxLevel}
			<button class="increment focus-template" on:click={() => (level += 1)}>
		...
		{/if}
		{#if level > minLevel}
			<button class="decrement focus-template" on:click={() => (level -= 1)}>
				<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" height="32" width="32">
					<path
		...
		{:else if onDelete}
			<button class="delete focus-template" on:click={onDelete}>
