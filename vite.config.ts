...
const config: UserConfig = {
	...
	build: {
		// The Windows webview (WebView2) is supposed to update itself automatically, so it's safe to target the latest 3 versions (i.e. Blink 111).
		...
		target: process.env.TAURI_PLATFORM == 'windows' ? 'edge111' : ['es2021', 'safari14'],
